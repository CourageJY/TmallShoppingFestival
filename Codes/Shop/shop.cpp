...
//店铺统一设置所有商品的属性
void Shop::setGoods(vector<string> names, vector<double> prices, vector<time_t> times){
    int number = this->goods.size();
    if (names.size() != number || prices.size() != number || (times.size() != 0 && times.size() != number))
        return;

    map<Goods, int>::iterator iter = this->goods.begin();
    for (int i = 0; i < number; i++) {
        ...
        tempGoods.setGoods(names[i], prices[i], times.size() == 0 ? NULL : times[i]);
...
void Shop::showGoods(){
    ...
    for (map<Goods, int>::iterator iter = this->goods.begin(); iter != this->goods.end(); iter++) {
...
void Shop::showComments(){
    cout << "本店评价：" << endl << endl;
    for (vector<Comment*>::iterator iter = this->comments.begin(); iter != this->comments.end(); iter++) {
        cout <<"    "<< (*iter)->getCustomer()->getName() << "：" << (*iter)->getContent() << endl << endl;
    }
    cout << "-------------------------------------------" << endl << endl;
}
